...
/*
    ...
    - DB에서는 데이터 개체 무결성을 지키기 위해서 기본키(PK) 제약조건을 사용한다
    ...
    - NOT NOULL : 해당 도메인에 null을 허용하지 않는다
    - PRIMARY KEY : 기본키, NOT NULL + UNIQUE, 한 테이블 당 하나만 설정 가능
    - FOREIGN KEY : 외래키, 해당 도메인을 외래키로 설정.
                    어느 컬럼을 참조할지 지정해야한다.
...
/*
    ...
    - DB사용자는 데이터 딕셔너리를 직접 수정할 수 없다
    
    # 데이터 딕셔너리 뷰(Data Dictionary View)
    
    - 데이터 딕셔너리를 사용자가 확인할 수 있게끔 제공하는 뷰(View)
        * 뷰(View) - DB 오브젝트 중 하나로 전체 데이터의 일부만 보기 좋은 형태로 
                    조회할 수 있는 미니 테이블 (원본 테이블의 부분 집합)
    
    - 앞에 user_를 붙이면 현재 접속 DB계정의 데이터 딕셔너리를 확인할 수 있다
...
/*
    ...
    - 제약조건 이름을 설정하지 않으면 오라클에서 생성한 자동 이름을 사용한다
       (어떤 잘못을해서 제약조건을 위배했는지 알기 힘들어진다)
    - "컬럼명 컬럼타입 CONSTRAINT 제약조건명 제약조건타입"을 통해 
      제약 조건의 이름도 직접 설정할 수 있다
...
CREATE TABLE coffee(
    coffee_id NUMBER(4) -- SYS000대신 내가 지정한 제약조건명을 사용할 수 있다
...
INSERT INTO coffee VALUES(1, '카페라떼', 2500, 'A');
INSERT INTO coffee VALUES(1, '카페라떼', -2500, 'V');
